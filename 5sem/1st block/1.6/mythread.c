...
void free_detached_thread() {
    munmap((void *)thread_global - STACK_SIZE + sizeof(mythread_struct),
           STACK_SIZE);
}
...
int start_routine_wrapper(void *args) {
    ...
    if (!(thread_global->cancelled) && !setjmp(thread_global->env)) {
        thread_global->retval =
            thread_global->start_routine(thread_global->args);
    }
...
void *create_stack() {
    ...
    void *stack = mmap(NULL, STACK_SIZE, PROT_NONE,
    ...
    err = mprotect(stack + PAGE, STACK_SIZE - PAGE, PROT_WRITE | PROT_READ);
    if (err != 0) {
...
int mythread_create(mythread_t *thread, void *(*start_routine)(void *),
    ...
    *thread = thread_stack + STACK_SIZE - sizeof(mythread_struct);
    ...
    int err;
    err = clone(
        start_routine_wrapper, *thread,
        CLONE_VM | CLONE_FS | CLONE_FILES | CLONE_SIGHAND | CLONE_THREAD |
            CLONE_SYSVSEM | CLONE_PARENT_SETTID | CLONE_CHILD_CLEARTID,
        (void *)*thread, &(*thread)->finished, NULL, &(*thread)->finished);
    if (err == -1) {
...
int mythread_join(mythread_t thread, void **retval) {
    ...
    int err = munmap((void *)thread - STACK_SIZE + sizeof(mythread_struct),
                     STACK_SIZE);
    if (err != 0) {
...
mythread_t mythread_self() { return thread_global; }
